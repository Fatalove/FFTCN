...
正式训练文件不得导入本模块。本参考只复用已经完成的步骤二数据接口和
里程碑 5–8 的教学重构模块，不导入原仓库 Loader、模型包装类或 trainer。
"""
...
def run_epoch(
    ...
    for inputs, targets in batches:
        ...
        # 所有位置的损失和，再加入分子，避免小 batch 与满 batch获得相同权重。
        ...
        # fit_stage 传入的是 tqdm 包装后的 Loader；每完成一个 batch，就把当前
        # 累计 loss 和 ACC 显示在进度行；两者只覆盖目前已经处理的位置。
        set_postfix = getattr(batches, "set_postfix", None)
        if callable(set_postfix):
            # refresh=False 只更新待显示字段，交给 tqdm 的正常刷新节奏统一绘制，
            # 避免每个 batch 为指标单独刷新一次终端而拖慢长训练。
            set_postfix(
...
def fit_stage(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # DataLoader 的 len() 是本轮 batch 总数；tqdm 每取完一个 batch 自动推进，
        # 因而进度单位与实际的“前向、反向、更新一次”完全一致。
        with tqdm(
            train_loader,
        ...
        # validation 也由多个 batch 组成，但 optimizer=None，所以只前向并汇总损失；
        # 单独的描述能明确当前已离开训练更新，正在计算 validation 选优依据。
        with tqdm(
            validation_loader,
